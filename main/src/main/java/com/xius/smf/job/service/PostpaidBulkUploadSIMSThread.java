...
public class PostpaidBulkUploadSIMSThread implements Runnable {
	private Logger logger = LogManager.getLogger("BulkUploadSIMSThread");
	...
	public void startUploadPrcocess() {
		...
		PostpaidActivationRestResponse postRestResponse = null;
		...
		if( null != uploadSIMInfo ) {
			...
			try {
				...
				String propflag=InitiateAll.getSMFProps().getProperty("invokeXbusRestApis");
					...
					if(propflag!=null && propflag.equalsIgnoreCase("Y") && uploadSIMInfo.getUser_input_field2()!=null && uploadSIMInfo.getUser_input_field2().equalsIgnoreCase("RESTAPI")){
						postRestResponse = invokeNormalPostPaidActRest( uploadSIMInfo );
					}
					else{
		...
		if( null != preResponse  || null != postResponse || null !=postRestResponse) {
			...
			} else if(null != postRestResponse){
			
				updateSIMStatusRest( postRestResponse, uploadSIMInfo );
			}
	...
	private void updateSIMStatus(PostpaidActivationResponse xBusResponse, BulkSIMSUploadCursorData uploadSIMInfo) {
		...
		if( null != xBusResponse ) {
			...
			domainData = setMOSRequest( xBusResponse,uploadSIMInfo  );
		...
		try {
			...
			spFactory = ServiceUtils.executeSPWithOutCommit("sim_update_pre_post", domainData, domainData);
...
//		// =================   ManagePostPaidGrpImpl  End=============
	...
	private void updateSIMStatusRest(PostpaidActivationRestResponse xBusResponse, BulkSIMSUploadCursorData uploadSIMInfo) {
		UpdateDispensedSIMData domainData = null;
		if( null != xBusResponse ) {
...
*/
	...
	private PostpaidActivationRestResponse invokeNormalPostPaidActRest(BulkSIMSUploadCursorData uploadSIMInfo) 
	throws MalformedURLException, ServiceException, InstantiationException, RemoteException {
		...
		logger.info(">>> Calling invokeNormalPostPaidActRest for SIM : " + sim + ", Tx Number: " + txNumber);
		
		PostpaidActivationRestRequest request = new PostpaidActivationRestRequest();
		...
		if( Utilities.isNull( uploadSIMInfo.getGender() ) == false ){
			...
			if( Utilities.isNull( gender ) == false ) {
				...
				logger.info("invokeNormalPostPaidActRest Gender feild from DB is NUL for SIM : " + sim + ", Tx Number: " + txNumber);
			}
		}
		if( Utilities.isNull( uploadSIMInfo.getDate_of_birth() ) == false ){
			personalInfo.setDateOfBirth(Utilities.covertStringToCalendar(uploadSIMInfo.getDate_of_birth().trim(), "GMT"));
		...
		*/
		...
		XBusProvisiongAppClient xClient = new XBusProvisiongAppClient();
		PostpaidActivationRestResponse response = xClient.postpaidActivationRest( request );
		
		logger.info("invokeNormalPostPaidActRest **** **** Time taken invokeXBus for SIM : " + sim + ", Tx Number: " + txNumber + " in (milli secons): " + (System.currentTimeMillis() - startTime) );
		return response;
	...
	private BulkSIMSUploadCursorData getUplodadInfo() {
		...
		try {
			...
			com.xius.smf.utils.ServiceUtils._executeStoredProcedure("pro_get_upload_sim_dtlspost", domainData, domainData);
		} catch (SMFAgentException e) {
	...
	private UpdateDispensedSIMData setMOSRequest( PostpaidActivationRestResponse xBusResponse) {
	...
	private UpdateDispensedSIMData setMOSRequest( PostpaidActivationResponse xBusResponse,BulkSIMSUploadCursorData uploadSIMInfo ) {
		...
		if( null != xBusResponse ) {
			...
			if(  StatusCode.SUCCESS  == xBusResponse.getStatusCode() 
				...
				try {
					...
					logger.error( "SMFAgentException in invoking tiaRegNotificationInvoker \n" + Utilities.getStackTrace(e));
				}
	...
	private UpdateDispensedSIMData setMOSRequest( PostpaidActivationRestResponse xBusResponse,BulkSIMSUploadCursorData uploadSIMInfo ) {
	...
	public PostpaidBulkUploadSIMSThread(){

	}

	public PostpaidBulkUploadSIMSThread(String txNumber, String sim){
