...
public class DeActivateSubscriberRetryProcessor implements Runnable {
	private	 static final Logger logger = LogManager.getLogger(DeActivateSubscriberRetryProcessor.class.getSimpleName());

	CountDownLatch latch = null;
	StateChangeRetryCursorData cursorData=null;
	
	public DeActivateSubscriberRetryProcessor(CountDownLatch latch,StateChangeRetryCursorData curData)
	{
		this.latch=latch;
	...
	public void run() {
		logger.info("---------In DeActivateSubscriberRetryProcessor run------------");
		...
		try {
			...
			if(response!=null)
				try {
					...
					logger.error("SMFAgentException in DeActivateSubscriberRetryProcessor "+Utilities.getStackTrace(e));
				}
			...
			logger.error("MalformedURLException in DeActivateSubscriberRetryProcessor "+Utilities.getStackTrace(e));
		} catch (RemoteException e) {
			logger.error("RemoteException in DeActivateSubscriberRetryProcessor "+Utilities.getStackTrace(e));
		} catch (SOAPException e) {
			logger.error("SOAPException in DeActivateSubscriberRetryProcessor "+Utilities.getStackTrace(e));
		}
		finally {			
			latch.countDown();
		}
	...
	public DeactivateSubscriberRequest setRequest(StateChangeRetryCursorData cursorData)
	{
		DeactivateSubscriberRequest request=new DeactivateSubscriberRequest();
		logger.info("Setting Request For TIA DeActivateSubscriberRetryProcessor with "+cursorData.toString());
	...
	public DeactivateSubscriberResponse callTia(DeactivateSubscriberRequest request) throws AxisFault,MalformedURLException, RemoteException, SOAPException
	{
		...
		logger.info("Hitting TIA with the url from DeActivateSubscriberRetryProcessor"+url);
		TIAManagementBindingStub stub=new TIAManagementBindingStub(new URL(url),service);
		 ...
		 header.setActor(null);
			...
			logger.info("NetworkName read from Property File DeActivateSubscriberRetryProcessor "+nwName);
	...
	public void updateStatusDb(DeactivateSubscriberResponse response,StateChangeRetryCursorData cursorData) throws SMFAgentException
	{
		...
		logger.info("Setting Data to DeActivateSubscriberRetryProcessor pro_state_change_retry_upd"+data.toString());
	...
	public StateChangeRetryUpdataData setData(DeactivateSubscriberResponse response,StateChangeRetryCursorData cursorData)
	{
