...
public class SimSwapFrmSMFRetryProcessor implements Runnable {
	private static final Logger logger = LogManager.getLogger(SimSwapFrmSMFRetryProcessor.class.getSimpleName());
	...
	public SimSwapFrmSMFRetryProcessor(CountDownLatch latch,RetryFromSMFCursorData curData){
	...
	public void run() {
		...
		try {
			...
			if (response != null)
				try {
					...
					logger.error("SMFAgentException in SimSwapFrmSMFRetryProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
				}
		...
		logger.error("MalformedURLException in SimSwapFrmSMFRetryProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (RemoteException e) {
			
			logger.error("RemoteException in SimSwapFrmSMFRetryProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (SOAPException e) {
			
			logger.error("SOAPException in SimSwapFrmSMFRetryProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		}
		finally {			
			latch.countDown();
		}
	...
	public SIMSwapRequest setRequest(RetryFromSMFCursorData cursordata)
	{
	...
	public SIMSwapResponse callTia(
			SIMSwapRequest request, RetryFromSMFCursorData cursorData)
		...
		logger.info("Hitting TIA with the url from SimSwapFrmSMFRetryProcessor"+url);
	...
	public void updateStatusDb(SIMSwapResponse response,
		...
		logger.info("Setting Data to SimSwapRetryProcessor pro_mdn_imsi_statchng_rtry_upd"+data.toString());
	...
	public SimSwapRetryUpdateData setData(
			SIMSwapResponse response, RetryFromSMFCursorData cursorData) {
