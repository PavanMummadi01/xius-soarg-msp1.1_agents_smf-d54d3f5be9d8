...
import com.xius.smf.job.service.PostpaidBulkUploadSIMSThread;
import com.xius.smf.utils.InitiateAll;
import com.xius.smf.utils.SMFAgentConstants;
import com.xius.smf.utils.ServiceUtils;
import com.xius.smf.utils.Utilities;

public class PostpaidBulkUploadSIMSTask {
	private Logger logger = LogManager.getLogger("PostpaidBulkUploadSIMSTask");

	public void doJob() {

	

			logger.debug( "*** Invoking PostpaidBulkUploadSIMSTask");
			...
			String maxRetrysStr =InitiateAll.getSMFProps().getProperty( "POSTPAID_BULK_UPLOAD_SIMS_RETRY_MAX");
			int maxRetrys = 1;
			if( Utilities.isNull( maxRetrysStr ) == false ) {
				...
				try {
					...
					logger.error("Exception while parsing property POSTPAID_BULK_UPLOAD_SIMS_RETRY_MAX. so taking default value 1" );
				}
			} else {
				logger.info( "Property POSTPAID_BULK_UPLOAD_SIMS_RETRY_MAX is null. so taking default value 1");
			}
			...
			if( domainData != null && domainData.getPo_error_code() == 0 ) {
				...
				logger.info("### No data from LightGet sP clc_sim_bulkupload_pre_post.pro_get_upload_sim_dtls");
	...
	private void updateTxStatus(String txNumber, String status, String remarks ) {
		...
		try {
			...
			spFactory = ServiceUtils.executeSPWithOutCommit("update_upload_sim_dtls_pre_post", domainData, domainData);
		} catch (SMFAgentException e) {

			logger.error( "SMFAgentException in update_upload_sim_dtls_pre_post \n" + Utilities.getStackTrace(e));
		}
	...
	public BulkSIMSUploadData getLightGet(){
		...
		try {
			...
			logger.error( "SMFAgentException in pro_get_upload_sim_dtlspost \n" + Utilities.getStackTrace(e));
		}

		logger.info( "Error Code from pro_get_upload_sim_dtlspost ==>"+domainData.getPo_error_code());
		logger.info( "Error desc from pro_get_upload_sim_dtlspost==>"+domainData.getPo_error_desc());
	...
	public void startThreadPool(BulkSIMSUploadData domainData) {
		...
		String maxThreadsStr = InitiateAll.getSMFProps().getProperty( "POSTPAID_BULK_UPLOAD_SIMS_MAX_THREDS_IN_POOL");
		int maxThreads = 5;
		if( Utilities.isNull( maxThreadsStr ) == false ) {
			...
			try {
				...
				logger.error("Exception while parsing property POSTPAID_BULK_UPLOAD_SIMS_MAX_THREDS_IN_POOL. so taking default value 5" );
			}
		} else {
			logger.info( "Property POSTPAID_BULK_UPLOAD_SIMS_MAX_THREDS_IN_POOL is null. so taking default value 5 ");
		}

		long startTimeTotal = System.currentTimeMillis();
		tpExecutor = new ThreadPoolExecutor(maxThreads, maxThreads, 100000, TimeUnit.SECONDS, queue);
		PostpaidBulkUploadSIMSThread worker = null;
		...
		for( BulkSIMSUploadCursorData txItem:  txDataList) {
			...
			if( null != txItem ) {
				...
				if(logger.isInfoEnabled())
					logger.info( "PostpaidBulkUploadSIMSThread Starting process (get from DB, xbus invocation, status update) SIM :" + txItem.getSim_serial_no() );
 
				worker = new PostpaidBulkUploadSIMSThread( domainData.getPo_transaction_no(), txItem.getSim_serial_no());
		...
		logger.info( "<<<### ### ###>>>Total Time taken for Processing PostpaidBulkUploadSIMS  (Count: "+ txDataList.size() +") : " + totalTime );
	}
}
