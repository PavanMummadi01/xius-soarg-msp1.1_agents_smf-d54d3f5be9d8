...
import com.xius.tia.TIAManagement_xsd.SuspendSubscriberRequest;
import com.xius.tia.TIAManagement_xsd.SuspendSubscriberResponse;

public class SuspSubRetryFrmSMFProcessor  implements Runnable {

	private static final Logger logger = LogManager.getLogger(SuspSubRetryFrmSMFProcessor.class.getSimpleName());
	...
	public SuspSubRetryFrmSMFProcessor(CountDownLatch latch,RetryFromSMFCursorData curData){
	...
	public void run() {
		logger.info("---------In SuspSubRetryFrmSMFProcessor run------------transid "+cursorData.getRetry_transaction_id());
		SuspendSubscriberRequest request=null;
		SuspendSubscriberResponse response=null;
		...
		try {
			...
			if(response!=null)
				try {
					...
					logger.error("SMFAgentException in SuspSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
				}
			...
			logger.error("MalformedURLException in SuspSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (RemoteException e) {
			logger.error("RemoteException in SuspSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (SOAPException e) {
			logger.error("SOAPException in SuspSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		}
		finally {			
			latch.countDown();
		}
	...
	public SuspendSubscriberRequest setRequest(RetryFromSMFCursorData cursorData)
	{
		SuspendSubscriberRequest request=new SuspendSubscriberRequest();
		logger.info("Setting Request For TIA SuspSubRetryFrmSMFProcessor with "+cursorData.toString());
	...
	public SuspendSubscriberResponse callTia(SuspendSubscriberRequest request) throws AxisFault,MalformedURLException, RemoteException, SOAPException
	{
		SuspendSubscriberResponse response=new SuspendSubscriberResponse();
		...
		logger.info("Hitting TIA with the url from SuspSubRetryFrmSMFProcessor"+url);
		TIAManagementBindingStub stub=new TIAManagementBindingStub(new URL(url),service);
		 ...
		 header.setActor(null);
			...
			logger.info("NetworkName read from Property File SuspSubRetryFrmSMFProcessor "+nwName);
		...
		response=stub.suspendSubscriber(request);
	...
	public void updateStatusDb(SuspendSubscriberResponse response,RetryFromSMFCursorData cursorData) throws SMFAgentException
	{
		...
		logger.info("Setting Data to SuspSubRetryFrmSMFProcessor pro_mdn_imsi_statchng_rtry_upd"+data.toString());
	...
	public StateChangeRetryUpdataData setData(SuspendSubscriberResponse response,RetryFromSMFCursorData cursorData)
	{
