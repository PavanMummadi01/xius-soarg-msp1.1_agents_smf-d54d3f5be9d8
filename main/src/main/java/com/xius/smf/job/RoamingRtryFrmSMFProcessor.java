...
import com.xius.smf.domaindata.RoamingRetryUpdateData;
...
import com.xius.tia.TIAManagement_xsd.RoamingRequest;
import com.xius.tia.TIAManagement_xsd.RoamingResponse;

public class RoamingRtryFrmSMFProcessor implements Runnable {

	
	private static final Logger logger = LogManager.getLogger(RoamingRtryFrmSMFProcessor.class.getSimpleName());
	...
	CountDownLatch latch=null;
		...
		public RoamingRtryFrmSMFProcessor(CountDownLatch latch,RetryFromSMFCursorData curData){
		...
		public void run() {
			...
			RoamingRequest request=null;
			RoamingResponse response=null;
			...
			try {
				...
				if (response != null)
					try {
						...
						logger.error("SMFAgentException in RoamingRtryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
					}
			...
			logger.error("MalformedURLException in RoamingRtryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
			} catch (RemoteException e) {
				
				logger.error("RemoteException in RoamingRtryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
			} catch (SOAPException e) {
				
				logger.error("SOAPException in RoamingRtryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
			}
			finally {			
				latch.countDown();
			}
		...
		public RoamingRequest setRequest(RetryFromSMFCursorData cursordata)
		{
			RoamingRequest request=new RoamingRequest();
			
			RequestData reqdata=new RequestData();
			
			
			request.setICC(cursordata.getOld_sim());
			
			
			request.setIMSI(cursordata.getOld_imsi().toString());
			
			
			request.setMSISDN(cursordata.getOld_msisdn().toString());
			...
			if(cursordata.getActivityId().equalsIgnoreCase(InitiateAll.getSMFProps().getProperty(SMFAgentConstants.ROAMING_ACTIVATE_ACTIVITY_ID)))
			request.setAction("HIGH");
			else
			request.setAction("LOW");	
		...
		public RoamingResponse callTia(
				RoamingRequest request, RetryFromSMFCursorData cursorData)
				throws AxisFault,MalformedURLException, RemoteException, SOAPException {
			RoamingResponse response = new RoamingResponse();
			...
			logger.info("Hitting TIA with the url from RoamingRtryFrmSMFProcessor"+url);
			...
			logger.info("NetworkName read from Property File RoamingRtryFrmSMFProcessor: "+nwName);
			...
			response = stub.roaming(request);

			return response;
		}
		
		public void updateStatusDb(RoamingResponse response,
				RetryFromSMFCursorData cursorData) throws SMFAgentException {
			RoamingRetryUpdateData data = null;
			...
			if(!response.getResponseData().getReturnCode().equalsIgnoreCase("0") && cursorData.getNotification_flag().equalsIgnoreCase("Y") )
			{
				...
				if(cursorData.getActivityId().equalsIgnoreCase(InitiateAll.getSMFProps().getProperty(SMFAgentConstants.ROAMING_ACTIVATE_ACTIVITY_ID)))
				replyToSubscriber.setUserDefined3("HIGH");
				else
				replyToSubscriber.setUserDefined3("LOW");	
			...
			logger.info("Setting Data to RoamingRtryFrmSMFProcessor pro_mdn_imsi_statchng_rtry_upd"+data.toString());
		...
		public RoamingRetryUpdateData setData(
				RoamingResponse response, RetryFromSMFCursorData cursorData) {
			RoamingRetryUpdateData data = new RoamingRetryUpdateData();
