...
public class Manage5GDeprovProcessor implements Runnable {
	private static final Logger logger = LogManager.getLogger(Manage5GDeprovProcessor.class.getSimpleName());
	...
	public Manage5GDeprovProcessor(CountDownLatch latch,
			VOLTECursorData curData) {
		this.latch = latch;
		this.cursorData = curData;
	}

	public Manage5GDeprovProcessor(VOLTECursorData curData) {
		this.cursorData = curData;
	}

	@Override
	public void run() {
		logger.debug(">>>>> Manage5GDeprovProcessor run called ...");
		try {
			callProvision5G();
		} finally {
			if (latch != null)
	...
	public void callProvision5G() {
		...
		logger.debug(">>>>> Manage5GDeprovProcessor callProvisionVOLTE called ...");
		try {
			...
			if (cursorData != null) {
				...
				response = manage5GInvoker.manage5GDeprovision(request,
						headerDetails);
				...
				if (response != null && response.getErrors() != null) {
					...
					if (response.getErrors().length > 0) {
						logger.error("In Manage5GDeprovProcessor errorMessage Manage5GResponse for "
								+ cursorData.getMsisdn_no() + ">>"
								+ response.getErrors()[0].getErrorMessage());
						logger.error("In Manage5GDeprovProcessor errorCode Manage5GResponse for "
								+ cursorData.getMsisdn_no() + ">>"
								+ response.getErrors()[0].getErrorCode());
					}
				} else {
					logger.debug("In Manage5GDeprovProcessor Message Manage5GResponse for "
							+ cursorData.getMsisdn_no() + ">>SUCCESS");
				}

			} else {
				logger.debug(">>>>> Manage5GDeprovProcessor - CursorData  is Null ...");
			}
		} catch (Exception _exp) {
			logger.error("Manage5GDeprovProcessor - Manage5GResponse ---> Exception");
