...
public class SuspendSubscriberProcessor implements Runnable {

	private static final Logger logger = LogManager.getLogger(SuspendSubscriberProcessor.class.getSimpleName());
	...
	public SuspendSubscriberProcessor(CountDownLatch latch,
	...
	SMFSimSaleInsertProcessor  smfsimsaleinsertprocessor=new SMFSimSaleInsertProcessor(cursorData);
		logger.info("---------In SuspendSubscriberProcessor run------------");
		...
		try {
			...
			if (response != null)
				try {
					...
					logger.error("SMFAgentException in SuspendSubscriberProcessor "+Utilities.getStackTrace(e));
			...
			logger.error("MalformedURLException in SuspendSubscriberProcessor "+Utilities.getStackTrace(e));
			
		} catch (RemoteException e) {
			logger.error("RemoteException in SuspendSubscriberProcessor "+Utilities.getStackTrace(e));
			
		} catch (SOAPException e) {
			logger.error("SOAPException in SuspendSubscriberProcessor "+Utilities.getStackTrace(e));
	...
	public SuspendSubscriberRequest setRequest(HlrStateChngCurData cursorData) {
		SuspendSubscriberRequest request = new SuspendSubscriberRequest();
		logger.info("Setting Request For TIA SuspendSubscriberProcessor with "+cursorData.toString());
	...
	public SuspendSubscriberRequest setSMFRequest(SIMSaleData simSaleData) {
		SuspendSubscriberRequest request = new SuspendSubscriberRequest();
		logger.info("Setting Request For TIA SuspendSubscriberRequest with "+simSaleData.toString());
	...
	public SuspendSubscriberResponse callTia(SuspendSubscriberRequest request)
		...
		logger.info("Hitting TIA with the url from SuspendSubscriberProcessor"+url);
		...
		logger.info("NetworkName read from Property File SuspendSubscriberProcessor: "+nwName);
		c_node3 = node1.addChildElement("networkName");
		c_node3.addTextNode(nwName);
		
		c_node4 = node1.addChildElement("userId");
		c_node4.addTextNode("iba");
	...
	public void updateStatusDb(SuspendSubscriberResponse response,
		...
		logger.info("Setting Data to SuspendSubscriberProcessor pro_hlr_statechange_notif_upd"+data.toString());
	...
	public HlrStateChangeNotifUpdate setData(
			SuspendSubscriberResponse response, HlrStateChngCurData cursorData) {
	...
	public UpdateSIMSaleData setSMFData(SuspendSubscriberResponse response) {
		...
		if(InitiateAll.getSMFProps().getProperty(SMFAgentConstants.SMF_CURRENTSTATE_ID)!=null)
	...
	public void failUpdate(HlrStateChngCurData cursorData,String msg,SMFSimSaleInsertProcessor  smfsimsaleinsertprocessor,boolean smfflag)
	{
		...
		if(smfflag){
			logger.error("Setting Failure status to SuspendSubscriberProcessor");
