...
public class ActSubRetryFrmSMFProcessor implements Runnable {
	private static final Logger logger = LogManager.getLogger(ActSubRetryFrmSMFProcessor.class.getSimpleName());
	...
	public ActSubRetryFrmSMFProcessor(CountDownLatch latch,RetryFromSMFCursorData curData){
	...
	public void run() {
		...
		logger.info("---------In ActSubRetryFrmSMFProcessor run------------ transid "+cursorData.getRetry_transaction_id());
		...
		try {
			...
			if (response != null)
				try {
					...
					logger.error("SMFAgentException in ActSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
				}
		...
		logger.error("MalformedURLException in ActSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (RemoteException e) {
			
			logger.error("RemoteException in ActSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		} catch (SOAPException e) {
			
			logger.error("SOAPException in ActSubRetryFrmSMFProcessor transid "+cursorData.getRetry_transaction_id()+Utilities.getStackTrace(e));
		}
		finally {			
			latch.countDown();
		}
	...
	public ActivateSubscriberRequest setRequest(RetryFromSMFCursorData cursorData) {
		ActivateSubscriberRequest request = new ActivateSubscriberRequest();
		logger.info("Setting Request For TIA ActSubRetryFrmSMFProcessor with transid "+cursorData.getRetry_transaction_id()+cursorData.toString());
	...
	public ActivateSubscriberResponse callTia(
			ActivateSubscriberRequest request, RetryFromSMFCursorData cursorData)
		...
		logger.info("Hitting TIA with the url from ActSubRetryFrmSMFProcessor "+url);
		...
		logger.info("NetworkName read from Property File ActSubRetryFrmSMFProcessor: "+nwName);
	...
	public void updateStatusDb(ActivateSubscriberResponse response,
		...
		logger.info("Setting Data to ActSubRetryFrmSMFProcessor pro_mdn_imsi_statchng_rtry_upd"+data.toString());
	...
	public StateChangeRetryUpdataData setData(
			ActivateSubscriberResponse response, RetryFromSMFCursorData cursorData) {
