...
public class SuspendSubscriberRetryProcessor implements Runnable {
	private static final Logger logger = LogManager.getLogger(SuspendSubscriberRetryProcessor.class.getSimpleName());


	CountDownLatch latch = null;
	StateChangeRetryCursorData cursorData = null;

	public SuspendSubscriberRetryProcessor(CountDownLatch latch,
	...
	public void run() {
		...
		logger.info("---------In SuspendSubscriberRetryProcessor run------------");
		...
		try {
			...
			if (response != null)
				try {
					...
					logger.error("SMFAgentException in SuspendSubscriberRetryProcessor "+Utilities.getStackTrace(e));
			...
			logger.error("MalformedURLException in SuspendSubscriberRetryProcessor "+Utilities.getStackTrace(e));
			
		} catch (RemoteException e) {
			logger.error("RemoteException in SuspendSubscriberRetryProcessor "+Utilities.getStackTrace(e));
			
		} catch (SOAPException e) {
			logger.error("SOAPException in SuspendSubscriberRetryProcessor "+Utilities.getStackTrace(e));
	...
	public SuspendSubscriberRequest setRequest(StateChangeRetryCursorData cursorData) {
		SuspendSubscriberRequest request = new SuspendSubscriberRequest();
		logger.info("Setting Request For TIA SuspendSubscriberRetryProcessor with "+cursorData.toString());
	...
	public SuspendSubscriberResponse callTia(SuspendSubscriberRequest request)
		...
		logger.info("Hitting TIA with the url from SuspendSubscriberRetryProcessor"+url);
		...
		logger.info("NetworkName read from Property File SuspendSubscriberRetryProcessor: "+nwName);
	...
	public void updateStatusDb(SuspendSubscriberResponse response,
		...
		logger.info("Setting Data to SuspendSubscriberRetryProcessor pro_state_change_retry_upd"+data.toString());
	...
	public StateChangeRetryUpdataData setData(
			SuspendSubscriberResponse response, StateChangeRetryCursorData cursorData) {
