...
public class TTPCRFDelProvProcessor implements Runnable {
	private static final Logger logger = LogManager.getLogger(TTPCRFDelProvProcessor.class.getSimpleName());
	...
	public TTPCRFDelProvProcessor(CountDownLatch latch,
			TTPCRFCursorData curData) {
		this.latch = latch;
		this.cursorData = curData;
	}
	
	public TTPCRFDelProvProcessor(
			TTPCRFCursorData curData) {
		this.cursorData = curData;
	}
	public void run() {
		 ...
		 if(response!=null){
			 ...
			 if(response.getStatus()!=null){
				 if(response.getStatus()!=null && String.valueOf(response.getStatus()).equalsIgnoreCase(SMFAgentConstants.SUCCESS)){
					 ...
					 updatedData.setPi_config_error(SMFAgentConstants.PCRF_SUCCESS_FLAG);
					 logger.debug(">>>>> TTPCRFDelProvProcessor - DeletePCRFResponse  is SUCCESS So calling  ProvisionProcessor..."); 
					 TTPCRFSubscribeProcessor provisionProcessor=new TTPCRFSubscribeProcessor(cursorData);
					 provisionProcessor.callProvisionPCRF();
