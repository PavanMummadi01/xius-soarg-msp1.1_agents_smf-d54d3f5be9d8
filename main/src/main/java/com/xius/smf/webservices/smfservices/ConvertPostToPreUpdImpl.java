...
@Service("ConvertPostToPreUpdImpl")
public class ConvertPostToPreUpdImpl {
	private  static final Logger logger = LogManager.getLogger(ConvertPostToPreUpdImpl.class.getSimpleName());
	public ConvertPostToPreUpdResponse convertPostToPreUpd(
			ConvertPostToPreUpdRequest request,HeaderDetails header) throws RemoteException,
			SMFAgentException {
	...
	private ConvertPreToPostUpdData setRequestData(ConvertPostToPreUpdRequest request,HeaderDetails headerDetails) {
		...
		{
			domainData.setPi_current_state_id(Long.parseLong(request.getCurStateId()));	
		}
	...
	private ConvertPostToPreUpdResponse setWebserviceResponse(
