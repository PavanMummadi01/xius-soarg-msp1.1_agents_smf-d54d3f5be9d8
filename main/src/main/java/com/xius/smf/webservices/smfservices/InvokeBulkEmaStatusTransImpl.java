...
import com.xius.smf.domaindata.InvokeBulkEmaStatusCursorTrans;
import com.xius.smf.domaindata.InvokeBulkEmaStatusDataTrans;
...
@Service("InvokeBulkEmaStatusTransImpl")
public class InvokeBulkEmaStatusTransImpl {
	
	private  static final Logger logger = LogManager.getLogger(InvokeBulkEmaStatusTransImpl.class.getSimpleName());
	public InvokeBulkEmaCmdStatusResponse invokeBulkEmaCmdStatus(
		...
		InvokeBulkEmaStatusDataTrans domainData =null;
		
		domainData = setRequestData(request, headerDetails);
		
		SPFactory spfactory=ServiceUtils.executeSPWithOutCommit("pro_get_ema_cmd_req_dtls_trans", domainData, domainData);
	...
	private InvokeBulkEmaCmdStatusResponse setWebserviceResponse(
			InvokeBulkEmaStatusDataTrans domainData) {
		...
		if(domainData!=null){
			...
			InvokeBulkEmaStatusCursorTrans cur=null;
			
			if(domainData.getPo_ema_cmd_req_dtls()!=null&&domainData.getPo_ema_cmd_req_dtls().size()>0){
				
				emaCmdRequestDtlsType=new EmaCmdRequestDtlsType[domainData.getPo_ema_cmd_req_dtls().size()];
				
				for(int i=0;i<domainData.getPo_ema_cmd_req_dtls().size();i++){
					
					cur=domainData.getPo_ema_cmd_req_dtls().get(i);
	...
	private InvokeBulkEmaStatusDataTrans setRequestData(
			InvokeBulkEmaCmdStatusRequest request, HeaderDetails headerDetails) {
		
		InvokeBulkEmaStatusDataTrans data=new InvokeBulkEmaStatusDataTrans();
